...
private func commonMapFailure<OldResult: ResultType>(_ body: @escaping(Error) throws -> OldResult.Value) -> (OldResult) -> TaskResult<OldResult.Value> {
    return { oldResult in
        TaskResult {
...
extension TaskType {
    ...
    public func recover(upon executor: ExecutorType, _ body: @escaping(Error) throws -> SuccessValue) -> Task<SuccessValue> {
    ...
    ///
    /// - seealso: FutureType.map(upon:_:)
    public func recover(upon queue: DispatchQueue, _ body: @escaping(Error) throws -> SuccessValue) -> Task<SuccessValue> {
    ...
    public func recover(_ body: @escaping(Error) throws -> SuccessValue) -> Task<SuccessValue> {
